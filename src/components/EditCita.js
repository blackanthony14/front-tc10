...
function EditCita() {
  ...
  // const [todosEstudiantes, setEstudiantes] = useState("");

  const querySearch = async (val) => {
    const value = valor;
    console.log(value);
    const res = await fetch(`https://tc9.herokuapp.com/cita/${value}`);
  ...
  const postAPI = async (datas, callback) => {
    ...
    const res = await axios
      .put(`https://tc9.herokuapp.com/cita/${valor}`, datas)
      .then((res) => {
        callback(null);
        setFetched(true);
      })
  ...
  const handleSubmit = (numero) => {
    postAPI(datos, setDatos, numero);
  };
  return (
    <Box
      className="App"
    ...
    >
      ...
      <div className="inputContainer">
        <input
          type="text"
          onChange={(e) => handleInputChange("metodoContacto", e)}
          value={datos?.metodoContacto}
        />
      ...
      <div className="inputContainer">
        <input
          type="number"
          onChange={(e) => handleInputChange("telefono", e)}
      ...
      <div className="inputContainer">
        <input
          type="email"
          onChange={(e) => handleInputChange("correo", e)}
          value={datos?.correo}
        />
      ...
      <div className="inputContainer">
        <input
          type="date"
          onChange={(e) => handleInputChange("fecha", e)}
          value={datos?.fecha}
        />
      ...
      <div className="inputContainer">
        <input
          type="time"
          onChange={(e) => handleInputChange("horas", e)}
          value={datos?.horas}
        />
